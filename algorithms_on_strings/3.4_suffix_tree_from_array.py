...
class SuffixTreeNode():
    def __init__(self, children, parent, strDepth, edgeStart, edgeEnd):
        self.parent = parent
        self.children = children
        self.strDepth = strDepth
        self.edgeStart = edgeStart
        self.edgeEnd = edgeEnd

def create_new_leaf(node, text, suffix):
    leaf = SuffixTreeNode(dict(), node, len(text) - suffix, suffix+node.strDepth, len(text)-1)
...
def suffix_array_to_suffix_tree(sa, lcp, text):
    ...
    root = SuffixTreeNode(dict(), None, 0, -1, -1)
    lcpPrev = 0
    curNode = root
    for i in range(len(text)):
        ...
        if curNode.strDepth == lcpPrev:
            curNode = create_new_leaf(curNode, text, suffix)
        else:
            edgeStart = sa[i-1] + curNode.strDepth
            offset = lcpPrev - curNode.strDepth
            midNode = break_edge(curNode, text, edgeStart, offset)
            curNode = create_new_leaf(midNode, text, suffix)
        if i < len(text) - 1:
            lcpPrev = lcp[i]
    ...
    while h:
        ...
        for _, child in sorted(node.children.items()):
            tree[idx].append((nodeCount, child.edgeStart, child.edgeEnd+1))
            tree[nodeCount] = []
            h.append((nodeCount, child))
            nodeCount += 1
...
if __name__ == '__main__':
    ...
    while len(stack) > 0:
        ...
        #print("%d %d" % (edges[edge_index][1], edges[edge_index][2]), "->", 
        #      text[edges[edge_index][1]:edges[edge_index][2]])
